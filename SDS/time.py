...
def get_past_unfinished_time_range(days_before=7):
    """
    Generates a range for old unfinished orders.
    Default: from 7 days ago 00:00:00 through the day before yesterday 23:59:59.
    """
    now = datetime.now()
    start_dt = (now - timedelta(days=days_before)).replace(hour=0, minute=0, second=0, microsecond=0)
    end_dt = (now - timedelta(days=2)).replace(hour=23, minute=59, second=59, microsecond=0)
